...
def parse_thien_di_section(full_text):

    text = re.sub(r'\\s*', '', full_text)

    # 1. Khoanh vùng phần 10. CUNG THIÊN DI 
    # Dùng Regex linh hoạt để bắt tiêu đề chương
    section_pattern = r'10\.\s+CUNG THIÊN DI.*?\n(.*?)(?=11\.\s+CUNG TẬT ÁCH|\Z)'
    section_match = re.search(section_pattern, text, re.DOTALL | re.IGNORECASE)

    if not section_match:
        return {"error": "Không tìm thấy chương 10. CUNG THIÊN DI"}
    ...
    desc_match = re.search(r'^(.*?)(?=10\.1\.|10\.2\.|[A-ZĐ]{2,})', section_text, re.DOTALL)
    description = desc_match.group(1).strip() if desc_match else ""
    description = re.sub(r'\s+', ' ', description)  # Chuẩn hóa khoảng trắng

    # 3. Tìm các khối sao (Star Blocks) 
    # Cấu trúc: 10.x. Tên Sao
    star_blocks = re.finditer(r'10\.\d+\.\s+([^\n]+)\n(.*?)(?=10\.\d+\.|\Z)', section_text, re.DOTALL)
...
def main():
    ...
    parsed_data = parse_thien_di_section(full_text)
    
    with open("data/data_process/tu_vi_boi_toan/cung/cung_thien_di_db.json", "w", encoding="utf-8") as f:
        json.dump(parsed_data, f, ensure_ascii=False, indent=2)
        
    print(f"Đã xuất thành công dữ liệu Cung Thiên Di ra file cung_thien_di_db.json!")
