...
def parse_phu_mau_section(full_text):
    
    # 1. Khoanh vùng chương 5. CUNG PHỤ MẪU
    section_pattern = r'5\.\s+CUNG\s+PHỤ\s+MẪU.*?\n(.*?)(?=5\.1\.\s+|\Z)'
    # 2. Lấy Description (đoạn đầu cho đến trước 5.1)
    ...
    if not section_match:
        return {"error": "Không tìm thấy chương 5. CUNG PHỤ MẪU"}
    ...
    # Giới hạn vùng tìm kiếm: từ mục "5.2. Ảnh hưởng..." đến mục "6. CUNG PHÚC ĐỨC"
    section_match = re.search(r'5\.2\.\s+Ảnh hưởng của những sao tọa thủ tại cung Phụ Mẫu(.*?)(?=6\.\s+CUNG PHÚC ĐỨC|\Z)', full_text, re.DOTALL)
    if not section_match:
        print("Không tìm thấy phần 5.2. Cung Phụ Mẫu.")
        return {"Cung_phu_mau": []}
        
    section_text = section_match.group(1)
    
    # Tìm các block sao (VD: "5.2.1. Tử Vi")
    star_blocks = re.finditer(r'5\.2\.(\d+)\.\s+([^\n]+)\n(.*?)(?=5\.2\.\d+\.|\Z)', section_text, re.DOTALL)
    ...
    return {
        "Cung_phu_mau": {
            ...
            "stars": stars_list,
            "source_content": re.search(r'5\.\s+CUNG\s+PHỤ\s+MẪU(.*?)(?=6\.\s+CUNG PHÚC ĐỨC|\Z)', full_text, re.DOTALL).group(1) if re.search(r'5\.\s+CUNG\s+PHỤ\s+MẪU(.*?)(?=6\.\s+CUNG PHÚC ĐỨC|\Z)', full_text, re.DOTALL) else ""  # Lưu nguyên văn nội dung để tham khảo sau này
        }
    }
...
def main():
    ...
    parsed_data = parse_phu_mau_section(full_text)
    
    with open("data/data_process/tu_vi_boi_toan/cung/cung_phu_mau_db.json", "w", encoding="utf-8") as f:
        json.dump(parsed_data, f, ensure_ascii=False, indent=2)
        
    print(f"Đã xuất thành công dữ liệu Cung Phụ Mẫu ra file cung_phu_mau_db.json!")
