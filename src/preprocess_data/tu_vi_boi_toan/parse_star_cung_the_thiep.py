...
def parse_the_thiep_section(full_text):

    text = re.sub(r'\\s*', '', full_text)

    # 1. Khoanh vùng phần 14. CUNG THÊ THIẾP (PHU QUÂN)
    # Dùng Regex linh hoạt để bắt tiêu đề chương
    section_pattern = r'14\.\s+CUNG THÊ THIẾP \(PHU QUÂN\).*?\n(.*?)(?=15\.\s+CUNG HUYNH ĐỆ|\Z)'
    section_match = re.search(section_pattern, text, re.DOTALL | re.IGNORECASE)

    if not section_match:
        return {"error": "Không tìm thấy chương 14. CUNG THÊ THIẾP (PHU QUÂN)"}
    ...
    desc_match = re.search(r'^(.*?)(?=14\.1\.|14\.2\.|[A-ZĐ]{2,})', section_text, re.DOTALL)
    description = desc_match.group(1).strip() if desc_match else ""
    # description = re.sub(r'\s+', ' ', description)  # Chuẩn hóa khoảng trắng

    # 3. Tìm các khối sao (Star Blocks) 
    # Cấu trúc: 14.x. Tên Sao
    star_blocks = re.finditer(r'14\.\d+\.\s+([^\n]+)\n(.*?)(?=14\.\d+\.|\Z)', section_text, re.DOTALL)
...
def main():
    ...
    parsed_data = parse_the_thiep_section(full_text)
    
    with open("data/data_process/tu_vi_boi_toan/cung/cung_the_thiep_db.json", "w", encoding="utf-8") as f:
        json.dump(parsed_data, f, ensure_ascii=False, indent=2)
        
    print(f"Đã xuất thành công dữ liệu Cung Thê Thiếp ra file cung_thê_thiếp_db.json!")
