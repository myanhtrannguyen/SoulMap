...
def parse_phuc_duc_section(full_text):
   
    # 1. Khoanh vùng chương 6. CUNG PHÚC ĐỨC
    section_pattern = r'6\.\s+CUNG\s+PHÚC\s+ĐỨC.*?\n(.*?)(?=6\.2\.\s+|\Z)'
    section_match = re.search(section_pattern, full_text, re.DOTALL | re.IGNORECASE)
    
    if not section_match:
        return {"error": "Không tìm thấy chương 6. CUNG PHÚC ĐỨC"}
    ...
    # 2. Lấy Description (đoạn đầu cho đến trước 6.1.1)
    desc_match = re.search(r'^(.*?)(?=6\.1\.)', section_text, re.DOTALL)
    description = desc_match.group(1).strip() if desc_match else ""

    # 3. Tìm các tiểu mục chính (6.1.1 và 6.1.2) để xác định context_type
    star_blocks = re.finditer(r'6\.1\.\d+\.\s+([^\n-]+)(?:\s*-\s*|\n)(.*?)(?=6\.1\.\d+\.|\Z)', section_text, re.DOTALL)
...
def main():
    ...
    parsed_data = parse_phuc_duc_section(full_text)
    
    with open("data/data_process/tu_vi_boi_toan/cung/cung_phuc_duc_db.json", "w", encoding="utf-8") as f:
        json.dump(parsed_data, f, ensure_ascii=False, indent=2)
        
    print(f"Đã xuất thành công dữ liệu Cung Phúc Đức ra file cung_phuc_duc_db.json!")
