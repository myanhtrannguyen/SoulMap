...
def parse_quan_loc_section(full_text):
    ...
    # 1. Khoanh vùng phần 12. CUNG TÀI BẠCH
    # Dùng Regex linh hoạt để bắt tiêu đề chương
    section_pattern = r'12\.\s+CUNG TÀI BẠCH.*?\n(.*?)(?=13\.\s+CUNG TỬ TỨC|\Z)'
    section_match = re.search(section_pattern, text, re.DOTALL | re.IGNORECASE)

    if not section_match:
        return {"error": "Không tìm thấy chương 12. CUNG TÀI BẠCH"}
    ...
    desc_match = re.search(r'^(.*?)(?=12\.1\.|12\.2\.|[A-ZĐ]{2,})', section_text, re.DOTALL)
    description = desc_match.group(1).strip() if desc_match else ""
    description = re.sub(r'\s+', ' ', description)  # Chuẩn hóa khoảng trắng

    # 3. Tìm các khối sao (Star Blocks) 
    # Cấu trúc: 12.x. Tên Sao
    star_blocks = re.finditer(r'12\.\d+\.\s+([^\n]+)\n(.*?)(?=12\.\d+\.|\Z)', section_text, re.DOTALL)
...
def main():
    ...
    with open("data/data_process/tu_vi_boi_toan/cung/cung_tai_bach_db.json", "w", encoding="utf-8") as f:
        json.dump(parsed_data, f, ensure_ascii=False, indent=2)
        
    print(f"Đã xuất thành công dữ liệu Cung Tài Bạch ra file cung_tai_bach_db.json!")
