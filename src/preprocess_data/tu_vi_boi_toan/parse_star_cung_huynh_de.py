...
def parse_huynh_de_section(full_text):
    
    # 1. Khoanh vùng chương 15. CUNG HUYNH ĐỆ
    section_pattern = r'15\.\s+CUNG\s+HUYNH\s+ĐỆ.*?\n(.*?)(?=15\.1\.\s+|\Z)'
    # 2. Lấy Description (đoạn đầu cho đến trước 15.1)
    ...
    if not section_match:
        return {"error": "Không tìm thấy chương 15. CUNG HUYNH ĐỆ"}
    ...
    # Giới hạn vùng tìm kiếm: từ mục "15.2. Nhận định ảnh hưởng các sao" đến mục "15.3. Cung Tử Tức"
    section_match = re.search(r'15\.2\.\s+Nhận định ảnh hưởng các sao(.*?)(?=16\.\s+NHẬN ĐỊNH VỀ MÙA SINH VÀ GIỜ SINH|\Z)', full_text, re.DOTALL)
    if not section_match:
        print("Không tìm thấy phần 15.2. Cung Huynh Đệ.")
        return {"Cung_huynh_đệ": []}
        
    section_text = section_match.group(1)
    
    # Tìm các block sao (VD: "13.2.1. Tử Vi")
    star_blocks = re.finditer(r'15\.2\.(\d+)\.\s+([^\n]+)\n(.*?)(?=15\.2\.\d+\.|\Z)', section_text, re.DOTALL)
    ...
    return {
        "Cung_huynh_đệ": {
            ...
            "stars": stars_list,
            "source_content": re.search(r'15\.\s+CUNG\s+HUYNH\s+ĐỆ(.*?)(?=16\.\s+NHẬN ĐỊNH VỀ MÙA SINH VÀ GIỜ SINH|\Z)', full_text, re.DOTALL).group(1) if re.search(r'15\.\s+CUNG\s+HUYNH\s+ĐỆ(.*?)(?=16\.\s+NHẬN ĐỊNH VỀ MÙA SINH VÀ GIỜ SINH|\Z)', full_text, re.DOTALL) else ""  # Lưu nguyên văn nội dung để tham khảo sau này
        }
    }
...
def main():
    ...
    parsed_data = parse_huynh_de_section(full_text)
    
    with open("data/data_process/tu_vi_boi_toan/cung/cung_huynh_de_db.json", "w", encoding="utf-8") as f:
        json.dump(parsed_data, f, ensure_ascii=False, indent=2)
        
    print(f"Đã xuất thành công dữ liệu Cung Huynh Đệ ra file cung_huynh_de_db.json!")
