...
def parse_quan_loc_section(full_text):
    ...
    # 1. Khoanh vùng phần 7. CUNG ĐIỀN TRẠCH
    # Dùng Regex linh hoạt để bắt tiêu đề chương
    section_pattern = r'7\.\s+CUNG ĐIỀN TRẠCH.*?\n(.*?)(?=8\.\s+CUNG QUAN LỘC|\Z)'
    section_match = re.search(section_pattern, text, re.DOTALL | re.IGNORECASE)

    if not section_match:
        return {"error": "Không tìm thấy chương 7. CUNG ĐIỀN TRẠCH"}
    ...
    desc_match = re.search(r'^(.*?)(?=7\.1\.|7\.2\.|[A-ZĐ]{2,})', section_text, re.DOTALL)
    description = desc_match.group(1).strip() if desc_match else ""
    description = re.sub(r'\s+', ' ', description)  # Chuẩn hóa khoảng trắng

    # 3. Tìm các khối sao (Star Blocks) 
    # Cấu trúc: 7.x. Tên Sao
    star_blocks = re.finditer(r'7\.\d+\.\s+([^\n]+)\n(.*?)(?=7\.\d+\.|\Z)', section_text, re.DOTALL)
...
def main():
    ...
    with open("data/data_process/tu_vi_boi_toan/cung/cung_dien_trach_db.json", "w", encoding="utf-8") as f:
        json.dump(parsed_data, f, ensure_ascii=False, indent=2)
        
    print(f"Đã xuất thành công dữ liệu Cung Điền Trạch ra file cung_dien_trach_db.json!")
